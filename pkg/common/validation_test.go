...
func TestGetVMReferenceFromDataDownload(t *testing.T) {
	tests := []struct {
		...
		{
			// Documents current behavior: neither the namespace annotation nor
			// Spec.SourceNamespace is required to be non-empty. An empty resolved
			// namespace is passed through as-is rather than rejected here --
			// callers that need a real backup manifest will simply fail later
			// (e.g. "no backup manifest found for VM <name>/<empty>") rather than
			// getting a clear "invalid VM reference" error at this point.
			name: "vm-name set, both namespace annotation and source namespace empty",
			dataDownload: &velerov2alpha1api.DataDownload{
				ObjectMeta: metav1.ObjectMeta{
					Name:      "test-dd",
